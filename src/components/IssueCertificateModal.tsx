...
export function IssueCertificateModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <Card className="relative w-full max-w-3xl p-6 z-10 shadow-2xl space-y-4 text-left max-h-[90vh] overflow-y-auto">
        <button
          onClick={onClose}
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted">
            Award <strong className="text-ink">{freelancerName}</strong> a publicly verifiable
            credential for &ldquo;{projectTitle}&rdquo;.
          </p>
        </div>
        ...
        {issuedId ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {skills.map((s) => (
                      <button
                        ...
                        disabled={issuing}
                        className="text-[10px] font-medium bg-surface-soft border border-hairline text-ink px-2 py-0.5 rounded-full cursor-pointer hover:border-border-strong"
                        title="Remove"
                      >
                        {s} ×
                      </button>
            ...
            <div className="space-y-1.5">
              ...
              <div className="border border-hairline rounded-[12px] overflow-hidden bg-white">
                ...
                <div className="px-4 py-5 space-y-3 text-center">
                  <div>
                    <p className="text-[9px] uppercase tracking-wider text-muted">This certifies that</p>
                    <p className="text-sm font-semibold text-ink">{freelancerName}</p>
                  </div>
                  <div>
                    <p className="text-[9px] uppercase tracking-wider text-muted">completed the role of</p>
                    <p className="text-xs font-semibold text-ink">
                      {roleTitle || <span className="text-border-strong">Role title…</span>}
                    </p>
                    <p className="text-[10px] text-body mt-0.5">on &ldquo;{projectTitle}&rdquo;</p>
                  </div>
                  {durationText && <p className="text-[10px] text-muted">{durationText}</p>}
                  {summary && (
                    <p className="text-[10px] text-body italic leading-relaxed">&ldquo;{summary}&rdquo;</p>
                  )}
                  {skills.length > 0 && (
                  ...
                  <div className="pt-2 border-t border-hairline">
                    <p className="text-[9px] uppercase tracking-wider text-muted">Issued by</p>
                    <p className="text-[10px] font-semibold text-ink">{companyName}</p>
                  </div>
                </div>
              </div>
            </div>
