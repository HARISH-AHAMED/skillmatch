...
export function ProjectsClient({
  ...
  projects: Project[];
  stats: { projects: number; openProjects: number; hires: number; totalReleased: number };
  ...
  const totalCommitted = PROJECTS.reduce((s, p) => s + (summaries[p.id]?.committed ?? 0), 0);
  ...
  const columns: Column<Project>[] = [
    ...
    },
    {
      ...
      render: (p) => {
        ...
        return formatMoney(s?.released ?? 0, s?.currency ?? "USD", true);
      },
    },
    {
      key: "applicants",
