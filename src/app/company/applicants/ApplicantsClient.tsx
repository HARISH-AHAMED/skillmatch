...
import type { Application, ApplicationStatus, Project } from "@/lib/types";
...
export function ApplicantsClient(props: {
  applications: Application[];
  projects: Project[];
  hiredByProject: Record<string, { roleId?: string; isApprentice: boolean }[]>;
}) {
  return (
    <Suspense fallback={null}>
      <ApplicantsView {...props} />
    </Suspense>
  );
}
...
function ApplicantsView({
  ...
  const roleOptions = useMemo(() => {
    ...
    return projects.find((p) => p.id === projectFilter)?.roles ?? [];
  ...
  const transition = (ids: string[], status: ApplicationStatus, message: string) => {
    const previous = ids.map((id) => [id, overrides[id]] as const);
    setOverrides((prev) => {
      const next = { ...prev };
    ...
    startTransition(async () => {
      ...
      try {
        // Each applicant is moved individually so one failure — a full role,
        // say — does not silently take the rest with it.
        ...
        toast.error(
          "Could not update those applicants",
      ...
      toast.success(
        message,
        `${ids.length} ${ids.length === 1 ? "applicant" : "applicants"} updated.`,
      );
    });
  };
  ...
  const hire = (a: Application) => {
    ...
    const capacity = getCapacity(project, hiredByProject[a.projectId] ?? [], a.roleId);
