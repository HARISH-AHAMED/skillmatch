...
export function ReviewForm({ projects, initialProjectId = "" }: ReviewFormProps) {
  ...
  return (
    <Card className="p-8 max-w-xl bg-white border border-[#E3E5EA]">
      {message && (
        <div
          className={`p-3.5 mb-6 rounded-lg text-xs font-semibold border ${
      ...
      {projects.length === 0 ? (
        <p className="text-[#5B6272] text-xs py-4">No active or completed contracts to review.</p>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {hiredFreelancers.length > 1 ? (
            ...
            <div className="p-3.5 bg-[#F8F9FB] rounded-lg border border-[#E3E5EA] text-xs">
              <span className="text-[11px] text-[#5B6272] block">Freelancer details</span>
              <strong className="text-[#1A1D29] font-semibold">{hiredFreelancers[0].user.name}</strong>
            </div>
          ) : null}

          {/* Star Rating select */}
          <div className="space-y-1.5">
            <label className="block text-xs font-semibold text-[#5B6272]">Rating Score (1-5)</label>
            <div className="flex gap-1.5">
              {[1, 2, 3, 4, 5].map((star) => (
                <button
                  ...
                  className="p-1 rounded-full hover:bg-[#E8F1FE] transition-colors cursor-pointer"
                  disabled={loading}
                >
                  <Star
                    className={`h-6 w-6 ${
                      star <= rating ? "text-[#8F5E08] fill-[#B9790A]/25" : "text-[#2159C9]"
                    }`}
                  />
                </button>
              ))}
          ...
          <div className="space-y-1.5">
            <label className="block text-xs font-semibold text-[#5B6272]">Written Feedback</label>
            <textarea
