...
import { closeProject, deleteProject, toggleProjectVisibility } from "@/actions/projectActions";
import { isProjectMutable, type FinancialSummary } from "@/lib/domain";
        ...
        ? [
            {
              ...
              onClick: () => {
                const wasHidden = hiddenIds.includes(p.id) || !p.isVisible;
                setHiddenIds((prev) =>
                  prev.includes(p.id) ? prev.filter((x) => x !== p.id) : [...prev, p.id],
                );
                startTransition(async () => {
                  const result = await toggleProjectVisibility(p.id);
                  if (!result.success) {
                    setHiddenIds((prev) =>
                      wasHidden ? [...prev, p.id] : prev.filter((x) => x !== p.id),
                    );
                  ...
                  <div className="flex flex-1 flex-col p-4">
                    ...
                    <dl className="mt-3.5 grid grid-cols-3 gap-2 border-y border-[var(--color-border-subtle)] py-3">
                      ...
                      <div>
                        ...
                        <dd className="mt-0.5 text-[14px] font-semibold tabular-nums text-[var(--color-text-primary)]">
                          {formatMoney(summary?.released ?? 0, summary?.currency ?? "USD", true)}
                        </dd>
                      </div>
                    </dl>

                    {p.compensation.type !== "UNPAID" && p.status !== "DRAFT" && (
                      <Progress className="mt-3" value={summary?.progress ?? 0} size="sm" />
                    )}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          startTransition(async () => {
            ...
            if (!result.success) {
              toast.toast({ title: result.error ?? "Could not remove the project", tone: "error" });
              return;
            }
            router.refresh();
            toast.toast({
              title: target.status === "DRAFT" ? "Draft archived" : "Project cancelled",
              description:
