...
export function CertificateDesignerClient({
  project,
  hired,
}: {
  project: Project;
  ...
  const save = async () => {
    setSaving(true);
    try {
      // The stored template uses null for an absent image; the UI uses undefined.
      await saveCertificateDesign(project.id, {
        ...config,
        logoUrl: config.logoUrl ?? null,
        signatureUrl: config.signatureUrl ?? null,
        signature2Url: config.signature2Url ?? null,
      });
      router.refresh();
      toast.success(
        "Template saved",
      ...
      toast.toast({
        title: error instanceof Error ? error.message : "Could not save the template",
        tone: "error",
      });
    } finally {
      setSaving(false);
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button
              disabled={!issueForm.applicationId}
              onClick={() => {
                const recipient = hired.find((h) => h.id === issueForm.applicationId);
                setIssueOpen(false);
                if (!recipient) return;
                startTransition(async () => {
                  const result = await issueCertificate({
                    projectId: project.id,
                    freelancerId: recipient.freelancerId,
                    roleTitle: issueForm.roleTitle,
                    // Snapshotted from the listing, which is what the
                    // certificate attests to.
                    skills: project.requiredSkills,
                    durationText: issueForm.durationText || undefined,
                    summary: issueForm.summary || undefined,
                  });
                  if (!result.success) {
                    toast.toast({
                      title: result.error ?? "Could not issue the certificate",
                      tone: "error",
                    });
                    return;
                  }
      ...
      >
        <div className="flex flex-col gap-4">
          <Field label="Recipient" required>
            <Select
              value={issueForm.applicationId}
              onChange={(e) => {
                const next = hired.find((h) => h.id === e.target.value);
                setIssueForm((f) => ({
                  ...f,
                  applicationId: e.target.value,
                  roleTitle: next?.roleName ?? "Project Contributor",
                }));
              }}
            >
              {hired.map((h) => (
          ...
          <Field label="Role title on the certificate" help="Defaults to their role name.">
            <Input
              value={issueForm.roleTitle}
              onChange={(e) => setIssueForm((f) => ({ ...f, roleTitle: e.target.value }))}
            />
          </Field>
          <Field label="Duration">
            <Input
              value={issueForm.durationText || project.duration}
              onChange={(e) => setIssueForm((f) => ({ ...f, durationText: e.target.value }))}
          ...
          >
            <Textarea
              ...
              onChange={(e) => setIssueForm((f) => ({ ...f, summary: e.target.value }))}
