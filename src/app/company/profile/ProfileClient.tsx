...
export function ProfileClient({ company }: { company: Company }) {
  ...
  const save = async () => {
    ...
    try {
      await updateCompanyDetailedProfile({
        ...
        email: form.email,
        phone: form.phone,
        missionVision: form.missionVision,
        workCulture: form.workCulture,
      ...
      toast.toast({
        title: error instanceof Error ? error.message : "Could not save your company profile",
        tone: "error",
      });
    } finally {
      setSaving(false);
  ...
  const pickImage = async (file: File | undefined, key: "logoUrl" | "bannerUrl") => {
    if (!file) return;
  ...
  const addGalleryPhotos = async (files: FileList | null) => {
    ...
    const urls = uploads.flatMap((r) => ("url" in r ? [r.url] : []));
    const failed = uploads.length - urls.length;
    if (urls.length) {
      setGallery((g) => [...g, ...urls]);
      setDirty(true);
    }
    if (failed > 0) {
      toast.toast({
        title: `${failed} ${failed === 1 ? "photo" : "photos"} could not be uploaded`,
        tone: "error",
      });
    }
  };
              ...
              <Card padding="none" className="overflow-hidden">
                <div className="relative h-32 bg-[var(--color-surface-sunken)] md:h-40">
                  ...
                  <input
                    ...
                    className="hidden"
                    onChange={(e) => void pickImage(e.target.files?.[0], "bannerUrl")}
                  />
                  <button
                    type="button"
                  ...
                  <div className="-mt-10 flex items-end gap-4">
                    <div className="relative">
                      ...
                      <input
                        ...
                        accept="image/png,image/jpeg,image/webp"
                        className="hidden"
                        onChange={(e) => void pickImage(e.target.files?.[0], "logoUrl")}
                      />
                      <button
                        type="button"
          ...
          {tab === "gallery" && (
            <Card padding="lg">
              ...
              <button
                ...
                className="mt-4 w-full rounded-[var(--radius-md)] border border-dashed border-[var(--color-border-emphasis)] bg-[var(--color-surface-alt)] p-6 text-center"
              >
                <input
                  ref={galleryInput}
                  type="file"
                  ...
                  accept="image/png,image/jpeg,image/webp"
                  className="hidden"
                  onChange={(e) => void addGalleryPhotos(e.target.files)}
                />
