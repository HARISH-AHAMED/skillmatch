...
import { acceptsApplications, getCapacity, type CapacityView } from "@/lib/domain";
...
export function ApplyClient({
  ...
  matchScore: { aiScore: number; breakdown: Application["scoreBreakdown"] };
  ...
  const capacity: CapacityView = getCapacity(project, hired, roleId || undefined);
  ...
  const submit = () => {
    ...
    void (async () => {
      ...
      if (!result.success) {
        setErrors([result.error ?? "Could not submit your application."]);
        window.scrollTo({ top: 0, behavior: "smooth" });
        return;
      }
      ...
      toast.success(
        ...
        `${project.company.companyName} has been notified with your ${matchScore.aiScore}% match score.`,
      );
      window.scrollTo({ top: 0, behavior: "smooth" });
    })();
  };
