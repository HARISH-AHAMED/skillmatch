...
export function CertificatesClient({ certificates }: { certificates: Certificate[] }) {
  ...
  // Seeded from what the backend already stores, then kept in step with each
  // visibility toggle the action confirms.
  ...
  const isHidden = (c: Certificate) => hidden.includes(c.id);

  const setVisibility = (certificateId: string, visible: boolean) => {
    setHidden((prev) =>
      visible ? prev.filter((id) => id !== certificateId) : [...prev, certificateId],
    );
    startTransition(async () => {
      const result = await setCertificateVisibility(certificateId, visible);
      if (result.success) {
        toast.toast({
          title: visible ? "Shown on your profile" : "Hidden from your profile",
          tone: "success",
        });
        return;
      }
      setHidden((prev) =>
        visible ? [...prev, certificateId] : prev.filter((id) => id !== certificateId),
      );
      toast.toast({ title: result.error ?? "Could not update visibility", tone: "error" });
    });
  };
