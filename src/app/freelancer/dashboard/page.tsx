...
export default function FreelancerDashboardPage() {
  ...
  const data = useMemo(() => {
    ...
    const apps = applicationsForFreelancer(freelancer.id);
    return {
      stats: freelancerDashboardStats(freelancer.id),
      apps,
      active: apps.filter(
        (a) =>
          a.status === "HIRED" &&
      ...
      needsAction: apps.filter(
        (a) =>
          (a.status === "HIRED" && !a.teamConfirmedAt) ||
          (a.offer && a.offer.status === "PENDING") ||
          (a.interview && a.interview.status === "SCHEDULED"),
      ),
      recommended: recommendedProjectsFor(freelancer.id, 4),
      workspaces: session ? workspacesForUser(session.userId, "FREELANCER") : [],
            ...
            {data.workspaces.length === 0 ? (
              ...
              <ul className="flex flex-col gap-3">
                {data.workspaces.map((w) => {
                  ...
                  const fin = app ? getApplicationFinancials(app.id) : null;
                  return (
