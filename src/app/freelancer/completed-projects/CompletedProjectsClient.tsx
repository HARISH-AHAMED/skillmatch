...
import type { Application, Certificate, Freelancer, Review } from "@/lib/types";
...
export function CompletedProjectsClient({
  ...
  const data = useMemo(() => {
    const earnings = completed.reduce(
      (s, a) => s + (financials[a.id]?.totalReleased ?? 0),
      0,
    );
    return { completed, certificates, written, received, earnings };
  }, [completed, certificates, written, received, financials]);
                      ...
                      <dl className="mt-4 grid gap-3 sm:grid-cols-3">
                        <div className="rounded-[var(--radius-sm)] bg-[var(--color-surface-alt)] p-2.5">
                          ...
                          <dd className="mt-0.5 text-[14px] font-semibold tabular-nums text-[var(--color-text-primary)]">
                            {formatMoney(fin?.totalReleased ?? 0, app.project.compensation.currency)}
                          </dd>
                        </div>
                        <div className="rounded-[var(--radius-sm)] bg-[var(--color-surface-alt)] p-2.5">
              ...
              onClick={() => {
                ...
                startTransition(async () => {
                  try {
                    ...
                    toast.toast({
                      title: error instanceof Error ? error.message : "Could not submit your review",
                      tone: "error",
                    });
                  }
                });
