...
import { confirmTeamMatch } from "@/actions/roleActions";
import {
  negotiateOfferAction,
  respondToOfferLetterAction,
  signDigitalContract,
} from "@/actions/workflowActions";
import { getProjectTeam } from "@/lib/domain";
...
export function ApplicationDetailClient({
  ...
  const team = getProjectTeam(project, hired);

  /** Runs one action, then re-reads the page from the server on success. */
  const run = async (
    ...
    toast.toast({ title: result.error ?? "That action could not be completed", tone: "error" });
  };
  ...
  const teammates = hired.filter((a) => a.id !== application.id);
            ...
            <Card padding="lg" className="border-[var(--color-brand)] bg-[var(--color-brand-softer)]">
              ...
              <div className="flex flex-wrap gap-2">
                <Button
                  ...
                  onClick={() =>
                    void run(
                      () => confirmTeamMatch(application.id, "CONFIRM"),
                      () => {
        ...
        footer={
          <>
            ...
            <Button
              onClick={() =>
                void run(
                  () => respondToOfferLetterAction(application.id, "ACCEPT"),
                  () => {
                ...
                void run(
                  () =>
                    negotiateOfferAction(
                      ...
                      // The offer keeps its billing model; only the amount is
                      // under negotiation here.
                      (application.offer?.category ?? "FIXED") as Parameters<
                        typeof negotiateOfferAction
                      >[2],
                      application.offer?.currency,
                      counterMessage,
                    ),
        ...
        footer={
          <>
            ...
            <Button
              ...
              onClick={() =>
                void run(
                  () => respondToOfferLetterAction(application.id, "DECLINE", declineReason),
                  () => {
        ...
        footer={
          <>
            ...
            <Button
              ...
              onClick={() =>
                void run(
                  () => confirmTeamMatch(application.id, "DECLINE", declineReason),
                  () => {
